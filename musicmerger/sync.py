...
def prepare(folder, job, *, language='auto', align_model=None, vocals='auto'):
    ...
    # A longer ASR model is a bounded fallback for missing lyric anchors only.
    if any(line.get('wstart') is None for line in lines):
        print('Ada lirik tanpa anchor; coba transkripsi medium sekali.', flush=True)
        words = karaoke.whisper_words(analysis, support / 'asr-medium-cache.json',
                                      language=detected, model_name='medium')
        lines = karaoke.build_line_timing(lyrics, words)
    if any(line.get('wstart') is None for line in lines):
        raise ValueError('Lirik tidak dapat dipetakan lengkap setelah dua percobaan. '
                         'Pastikan MD berisi lirik yang benar-benar dinyanyikan, termasuk pengulangan.')
    ...
    reference.write_text(json.dumps(dict(audio=karaoke.audio_fingerprint(audio),
        ...
        lines=lines, language=detected, separation=separation), ensure_ascii=False, indent=2), encoding='utf-8')
...
def main():
    ...
    try:
        prepare(args.folder.resolve(), args.job.resolve(), language=args.language,
                align_model=args.align_model, vocals=args.vocals)
